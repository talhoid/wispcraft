const PROTOCOL_CLIENT_VERSION = 0x01;
const PROTOCOL_SERVER_VERSION = 0x02;
const PROTOCOL_CLIENT_REQUEST_LOGIN = 0x04;
const PROTOCOL_SERVER_ALLOW_LOGIN = 0x05;
const PROTOCOL_CLIENT_PROFILE_DATA = 0x07;
const PROTOCOL_CLIENT_FINISH_LOGIN = 0x08;
const PROTOCOL_SERVER_FINISH_LOGIN = 0x09;
...
function makePacket(packetId, dataByteArray) {
    ...
    return [...makeVarInt(packetIdVarInt.length + dataByteArray.length), ...packetIdVarInt, ...dataByteArray];
}
...
async function makeCompressedPacket(packetId, dataByteArray, thresh) {
    ...
    if (thresh >= 0 && toCompress.length > thresh) {
        ...
        return [...makeVarInt(lenUncompressed.length + chunks.length), ...lenUncompressed, ...chunks];
    } else {
        const lenUncompressed = makeVarInt(0);
        return [...makeVarInt(lenUncompressed.length + toCompress.length), ...lenUncompressed, ...toCompress];
    }
}
